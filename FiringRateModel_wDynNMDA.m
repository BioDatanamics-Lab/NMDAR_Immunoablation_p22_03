function [Inside,Outside,HW]=FiringRateModel_wDynNMDA(NMDA,EIratio,noise)
...
elseif KSE == 2
    ...
    s1 = 0.*ones(1,N);%zeros(1,N);%1*ones(1,N);%zeros(1,N);
    s2 = 0.*ones(1,N);%zeros(1,M);
    s3 = 0.;
    ...
    Gnmdagl1 = NMDA*ones(1,N);%zeros(1,N);
    Gnmdagl2 = NMDA*ones(1,M);%zeros(1,M);
    Gnmdagl3 = NMDA;
    ...
    Inside=nanmean(V([HW1:HW2]));
    Outside=nanmean([V([1:HW1]),V([HW2:end])]);
